...
<body>
    <div class="container mt-4">
        ...
        @if($jobs->isEmpty())
            ...
            <br>
            <a href="{{ route('find_job') }}" class="btn btn-primary">กลับไปที่หน้าหางาน</a>
        @else
            @php
                $job = $jobs->first();
            @endphp
            ...
            <form action="{{ route('chat', ['id' => $job->id]) }}" method="GET">
                @csrf
                <br>
                <td class="form-group">
                ...
                <td class="form-group">
                    <a href="{{ route('find_job') }}" class="btn btn-danger">ไม่รับงาน</a>
                </td>
            </form>
        @endif
    </div>
